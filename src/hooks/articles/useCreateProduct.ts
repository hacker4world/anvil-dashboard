...
export function useCreateProduct(
  ...
  const onSubmit = async (data: CreateProductFormData) => {
    ...
    try {
      ...
      const response = await productClient.create(payload);
      const createdProduct = response.data.data;

      useProductStore
        .getState()
        .addArticle(productToArticleModel(createdProduct));
      ...
      if (axios.isAxiosError(err)) {
        if (err.response) {
          const apiMessage = (err.response.data as { message?: string })
            ?.message;
          setError(apiMessage || "Erreur lors de la création du produit");
        } else {
          setError(
            "Une erreur réseau est survenue. Veuillez vérifier votre connexion et réessayer.",
          );
