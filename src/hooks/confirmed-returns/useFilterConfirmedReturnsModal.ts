...
interface UseFilterConfirmedReturnsModal {
...
export function useFilterConfirmedReturnsModal(
  onClose: () => void,
): UseFilterConfirmedReturnsModal {
  const storeFilters = useConfirmedReturnStore((s) => s.filters);
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      ...
      await useConfirmedReturnStore.getState().applyFilters(filters);
  ...
  const handleReset = useCallback(async () => {
    ...
    setAccountInput("");
    try {
      await useConfirmedReturnStore.getState().clearFilters();
