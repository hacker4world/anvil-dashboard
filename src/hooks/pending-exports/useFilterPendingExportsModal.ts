...
interface UseFilterPendingExportsModalReturn {
  // Local form state
...
export function useFilterPendingExportsModal(
  onClose: () => void,
): UseFilterPendingExportsModalReturn {
  const storeFilters = usePendingExportStore((s) => s.filters);
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      ...
      await usePendingExportStore.getState().applyFilters(filters);
  ...
  const handleReset = useCallback(async () => {
    ...
    try {
      await usePendingExportStore.getState().clearFilters();
