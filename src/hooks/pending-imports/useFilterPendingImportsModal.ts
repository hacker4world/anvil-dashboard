...
interface UseFilterPendingImportsModalReturn {
  // Local form state
...
export function useFilterPendingImportsModal(
  onClose: () => void,
): UseFilterPendingImportsModalReturn {
  const storeFilters = usePendingImportStore((s) => s.filters);
  ...
  const handleApply = useCallback(async () => {
    ...
    try {
      ...
      await usePendingImportStore.getState().applyFilters(filters);
  ...
  const handleReset = useCallback(async () => {
    ...
    try {
      await usePendingImportStore.getState().clearFilters();
