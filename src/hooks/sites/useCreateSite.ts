...
import { createConstructionSite } from "@/services/construction-site.service";
import { listAccounts } from "@/services/account.service";
import { useConstructionSiteStore } from "@/store/constructionSiteStore";
import { useVerifiedAccountStore } from "@/store/verifiedAccountStore";
import { toast } from "sonner";
import { CreateConstructionSiteRequest } from "@/models/ConstructionSite.model";
...
interface UseCreateSiteReturn {
  form: ReturnType<typeof useForm<CreateSiteFormData>>;
  isLoading: boolean;
  error: string | null;
...
export function useCreateSite(onSuccess: () => void): UseCreateSiteReturn {
  ...
  const onSubmit = async (data: CreateSiteFormData) => {
    ...
    try {
      const payload: CreateConstructionSiteRequest = {
        name: data.name,
        address: data.address,
        managerId: data.managerId,
      };
      const response = await createConstructionSite(payload);
      const siteModel = mapToSiteModel(response.data);

      useConstructionSiteStore.getState().addSite(siteModel);
      ...
      if (axios.isAxiosError(err)) {
        if (err.response) {
          const apiMessage = (err.response.data as { message?: string })
            ?.message;
          setError(apiMessage || "Erreur lors de la création du chantier");
        } else {
          setError(
            "Une erreur réseau est survenue. Veuillez vérifier votre connexion et réessayer.",
          );
