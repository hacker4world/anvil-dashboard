...
function extractErrorMessage(err: unknown, defaultMessage: string): string {
  if (isAxiosError(err)) {
    ...
    if (!err.response) return "Une erreur réseau est survenue.";
  }
...
function buildFilters(
  ...
  const filters: ReturnFilters = { confirmed: false };
  if (searchQuery.trim()) {
...
export const usePendingReturnStore = create<PendingReturnState>()(
  (set, get) => ({
    ...initialState,
    ...
    removeReturn: (id: number) => {
      set((prev) => ({
        returns: prev.returns.filter((ret) => ret.id !== id),
        total: prev.total - 1,
      }));
    },
    ...
    // ── Advanced filters ──────────────────────────
    applyFilters: async (filters: ReturnFilters) => {
      set({
        filters,
        returns: [],
        page: 1,
